...
/**
 *   	\file       circuit_card.php
 *		\ingroup    modulegp
 *		\brief      Page to create/edit/view circuit
 */
...
dol_include_once('/modulegp/class/circuit.class.php');
dol_include_once('/modulegp/lib/modulegp_circuit.lib.php');
...
$contextpage = GETPOST('contextpage', 'aZ') ?GETPOST('contextpage', 'aZ') : 'circuitcard'; // To manage different context of search
$backtopage = GETPOST('backtopage', 'alpha');
$backtopageforcancel = GETPOST('backtopageforcancel', 'alpha');
//$lineid   = GETPOST('lineid', 'int');

// Initialize technical objects
$object = new Circuit($db);
$extrafields = new ExtraFields($db);
$diroutputmassaction = $conf->modulegp->dir_output.'/temp/massgeneration/'.$user->id;
$hookmanager->initHooks(array('circuitcard', 'globalcard')); // Note that conf->hooks_modules contains array
...
$permissiontoread = $user->rights->modulegp->circuit->read;
$permissiontoadd = $user->rights->modulegp->circuit->write; // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
$permissiontodelete = $user->rights->modulegp->circuit->delete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
$permissionnote = $user->rights->modulegp->circuit->write; // Used by the include of actions_setnotes.inc.php
$permissiondellink = $user->rights->modulegp->circuit->write; // Used by the include of actions_dellink.inc.php
...
if ($action === "update") {
    ...
    $sql_cat = $db->query('DELETE FROM '. MAIN_DB_PREFIX . 'modulegp_categories_circuits WHERE fk_modulegp_circuits = '.$object->id);
...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/modulegp/circuit_list.php', 1);

	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) $backtopage = $backurlforlist;
			else $backtopage = dol_buildpath('/modulegp/circuit_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
		}
	}
	...
	$triggersendname = 'MODULEGP_CIRCUIT_SENTBYMAIL';
	$autocopy = 'MAIN_MAIL_AUTOCOPY_CIRCUIT_TO';
	$trackid = 'circuit'.$object->id;
...
{
	print load_fiche_titre($langs->trans("Circuit"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	$head = circuitPrepareHead($object);
	print dol_get_fiche_head($head, 'card', $langs->trans("Circuit"), -1, $object->picto);
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/modulegp/circuit_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
...
// Categories
	...
	if ($action != 'presend')
	{
		...
		if ($includedocgeneration) {
			...
			$genallowed = $user->rights->modulegp->circuit->read; // If you can read, you can build the PDF to read content
			$delallowed = $user->rights->modulegp->circuit->write; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('modulegp:Circuit', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('circuit'));
		...
		$morehtmlright = '<a href="'.dol_buildpath('/modulegp/circuit_agenda.php', 1).'?id='.$object->id.'">';
